...
// Resilient Lead Storage (MongoDB with In-Memory fallback)
let memoryLeads = [
  {
    id: 'lead_1',
    name: 'Rajesh Kumar',
...
const fetchAllLeads = async () => {
  try {
    if (mongoose.connection.readyState === 1) {
      return await Lead.find().sort({ timestamp: -1 });
    }
  } catch (e) {
    console.warn('MongoDB Lead fetch fallback to memory store');
  }
  return memoryLeads;
};
...
const createNewLead = async (leadData) => {
  try {
    ...
    console.warn('MongoDB Lead create fallback to memory store');
  }
...
app.post('/api/leads', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to write lead: ' + err.message });
  }
});
...
4. **ABUSIVE LANGUAGE:** If the user uses abusive language, strictly apologize politely and redirect to Ace Automation services.
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    console.error('Error in chatbot communication:', error?.message || error);
    return res.json({
      text: "Namaste! Main Aace hoon, Ace Automation ka AI assistant. 👋\nMain aapki Website Development, AI Automation, AI Voice Agents aur CRM solutions mein madad kar sakta hoon. Aap humse seedha WhatsApp (+91 7000563768) par bhi sampark kar sakte hain!"
    });
  }
});
...
connectDB()
  ...
  .catch(err => {
    console.warn('⚠️ MongoDB connection warning (Server starting anyway):', err.message);
  })
  .finally(() => {
    app.listen(PORT, () => {
